...
读入时按 `Container+Carrier` 去重（保留首行），再按船公司分组。`ONEY` / `YMJA` 若都在输入中则先并行（默认可无头）；随后有头船公司按 `MSCU` → `MAEU` → `HLCU` → `CMDU` 串行。每家使用一个独立 worker 并按顺序逐箱查询；箱号会去掉空格和连字符并转大写，校验位不对仍会查询，只打日志警告。
